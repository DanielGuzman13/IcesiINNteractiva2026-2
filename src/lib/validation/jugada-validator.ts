import { Workspace, type Block } from 'blockly';
...
export class JugadaValidator {
  ...
  validarJugada(mode: ValidationMode = 'logica_disparo'): ValidationResult {
    ...
    const modeValidationResults =
      mode === 'logica_ciclo'
        ? [this.validarCicloContraataque(blocks)]
        : [this.validarLogicaDisparo(blocks)];

    const validationResults = [...baseValidationResults, ...modeValidationResults];
  ...
  private validarEstructuraBasica(blocks: Block[]): { isValid: boolean; messages: string[] } {
    const messages: string[] = [];
    let isValid = true;
  ...
  private validarBloquesAccion(blocks: Block[]): { isValid: boolean; messages: string[] } {
    const messages: string[] = [];
    let isValid = true;
  ...
  private validarLogicaDisparo(blocks: Block[]): { isValid: boolean; messages: string[] } {
    return this.validarContraPlantillas(blocks, 'logica_disparo');
  }

  private validarEstructuraIfElse(blocks: Block[]): { isValid: boolean; messages: string[] } {
    const messages: string[] = [];
    let isValid = true;
  ...
  private validarCicloContraataque(blocks: Block[]): { isValid: boolean; messages: string[] } {
    return this.validarContraPlantillas(blocks, 'logica_ciclo');
  }

  private validarContraPlantillas(
    blocks: Block[],
    mode: ValidationMode
    ...
    const coincide = templates.some((template) =>
      this.coincideSecuencia(parseResult.sequence, template.sequence)
    );
  ...
  private construirProgramaEstructurado(
    blocks: Block[]
  ): { isValid: boolean; messages: string[]; sequence: ProgramNode[] } {
    const messages: string[] = [];
    ...
    const finTopLevel = sequence.filter(
      (node) => node.kind === 'action' && node.action === 'fin'
    ).length;
    if (finTopLevel !== 1) {
  ...
  private parsearSecuencia(startBlock: Block | null, messages: string[]): ProgramNode[] {
  ...
  private parsearNodo(block: Block, messages: string[]): ProgramNode | null {
    switch (block.type) {
      case 'futbol_avanzar':
  ...
  private recorrerConexiones(block: Block | null, visitados: Set<string>): void {
  ...
  private generarPseudocodigo(blocks: Block[]): string {
    let pseudocode = '';
  ...
  private generarCodigoDesdeBloque(block: Block | null): string {
    let codigo = '';
    ...
    switch (block.type) {
      ...
      case 'futbol_si': {
        const conditionBlock = block.getInputTargetBlock('CONDITION');
        const conditionName = conditionBlock
          ? this.getConditionName(conditionBlock.type)
          : 'condición';
